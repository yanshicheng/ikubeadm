...
# 处理镜像的主函数
process_images() {
    local total_images=${#image_mapping[@]}
    ...
    # 从普通数组中提取源镜像和目标仓库信息
    for mapping in "${image_mapping[@]}"; do
        # 使用|作为分隔符拆分字符串
        image=$(echo "$mapping" | cut -d'|' -f1)
        subdirectory=$(echo "$mapping" | cut -d'|' -f2)
        
        # 提取镜像名和标签信息
        image_name_with_path=$(echo "$image" | awk -F':' '{print $1}')
        image_tag=$(echo "$image" | awk -F':' '{print $2}')
        image_name=$(echo "$image_name_with_path" | awk -F'/' '{print $NF}')
        
        new_image="$harbor_registry/$subdirectory/$image_name:$image_tag"
        
        processed_images=$((processed_images + 1))
        image_success=true
        
        print_separator
        echo -e "${BOLD}${CYAN}[$processed_images/$total_images] 处理镜像: $image${NC}"
        ...
        for platform in "${platforms[@]}"; do
            ...
            platform_tag="$new_image-$arch"
            
            echo -e "\n${BOLD}${YELLOW}[$processed_images/$total_images] ${arch}架构处理开始${NC}"
        ...
        # 创建多架构清单
        echo -e "\n${BOLD}${YELLOW}[$processed_images/$total_images] 创建多架构清单步骤${NC}"
        manifest_args="create --amend $new_image"
        for platform in "${platforms[@]}"; do
            arch=$(echo "$platform" | cut -d'/' -f2)
        ...
        # 清理本地镜像 (注释掉的部分保持不变)
        #echo -e "\n${BOLD}${YELLOW}[$processed_images/$total_images] 清理本地镜像${NC}"
        #for platform in "${platforms[@]}"; do
        #    arch=$(echo "$platform" | cut -d'/' -f2)
        ...
        if [ "$image_success" = true ]; then
            ...
            echo -e "\n${GREEN}${BOLD}[$processed_images/$total_images] 镜像 $image 处理成功!${NC}"
        else
            failed_count=$((failed_count + 1))
            echo -e "\n${RED}${BOLD}[$processed_images/$total_images] 镜像 $image 处理失败!${NC}"
        fi
